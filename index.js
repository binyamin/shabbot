...
module.exports = app => {
  app.on('issue_comment.created', async context => {
    ...
    if (comment.body.includes('@' + USER_LOGIN) && comment.user.login !== 'shabbot[bot]') {
      const resp = await (await fetch('https://www.hebcal.com/shabbat?cfg=json&zip=21208&m=50')).json()
      const shabbosTimeIn = moment(resp.items.filter(p => p.category === 'candles')[0].date).tz('America/New_York').subtract(BUFFER_BEFORE, 'minutes')
      const shabbosTimeOut = moment(resp.items.filter(p => p.category === 'havdalah')[0].date).tz('America/New_York')
